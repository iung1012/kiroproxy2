...
def validate_configuration() -> None:
    ...
    # Print errors — but don't abort; UI setup wizard handles unconfigured state
    if errors:
        logger.warning("")
        logger.warning("=" * 60)
        ...
        logger.warning("=" * 60)
        logger.warning("  Open http://localhost:8000/ui to complete setup")
        logger.warning("=" * 60)
        logger.warning("")
...
async def lifespan(app: FastAPI):
    ...
    if not all_accounts:
        logger.warning("No accounts configured — starting in setup mode. Open /ui to configure.")
    ...
    if not initialized:
        logger.warning("Failed to initialize any account — starting in setup mode. Open /ui to configure.")
        app.state.setup_required = True
        yield
        await app.state.http_client.aclose()
        return
...
# When no credentials are configured, block API requests and redirect to /ui
...
def print_startup_banner(host: str, port: int) -> None:
    ...
    print()
    print(f"  {DIM}UI Panel:      {url}/ui{RESET}")
